...
ec2_instance = template.add_resource(ec2.Instance(
    ...
    UserData=Base64(Join('', [
        ...
        "snap install yq\n",
        "apt update\n",
        "apt install neovim net-tools -y\n",
        "apt upgrade -y\n",
        ...
        "yq e '.server_url = \"https://",
        Ref(stack_name_parameter),
        ".",
        GetAtt(ssm_lambda_invocation, "DomainName"),
        ":443\"' -i /etc/headscale/config.yaml\n",
        "yq e '.listen_addr = \"0.0.0.0:443\"' -i /etc/headscale/config.yaml\n",
        "yq e '.acme_email = \"headscale@",
        GetAtt(ssm_lambda_invocation, "DomainName"),
        "\"' -i /etc/headscale/config.yaml\n",
        "yq e '.tls_letsencrypt_hostname = \"",
        Ref(stack_name_parameter),
        ".",
        GetAtt(ssm_lambda_invocation, "DomainName"),
        "\"' -i /etc/headscale/config.yaml\n",
        "yq e '.tls_letsencrypt_challenge_type = \"TLS-ALPN-01\"' -i /etc/headscale/config.yaml\n",
        "yq e '.base_domain = \"",
        Ref(magicdns_parameter),
        "\"' -i /etc/headscale/config.yaml\n",
        "f' -i /etc/headscale/config.yaml\n",
        "yq e '.dns_config.nameservers = [\"https://nextdns.io/",
